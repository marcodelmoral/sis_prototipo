...
class Command(BaseCommand):
    ...
    def procesa_shp(self, origen):
        ...
        entidades.to_postgis(Entidad._meta.db_table, self.engine, if_exists="append", index=False)

        self.stdout.write(self.style.NOTICE('Insertando municipios'))
        municipios.to_postgis(Municipio._meta.db_table, self.engine, if_exists="append", index=False)

        self.stdout.write(self.style.NOTICE('Insertando localidades'))
        localidades.to_postgis(Localidad._meta.db_table, self.engine, if_exists="append", index=False)

        self.stdout.write(self.style.NOTICE('Insertando localidades puntuales'))
        localidades_puntual.to_postgis(Localidad._meta.db_table, self.engine, if_exists="append", index=False)

        self.stdout.write(self.style.NOTICE('Insertando manzanas'))
        manzanas.to_postgis(Manzana._meta.db_table, self.engine, if_exists="append", index=False)
