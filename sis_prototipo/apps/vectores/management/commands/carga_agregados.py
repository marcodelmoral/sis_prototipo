...
class Command(BaseCommand):
    help = "Carga series de tiempo de vectores y clima"
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            nargs="?",
            type=str,
            help="Archivo de datos agregados",
            default="data/agregados/serie_final.csv",
        )

    def carga_datos(self, archivo):
        df = pd.read_csv(
            archivo, index_col=None, low_memory=False, encoding="latin-1"
        )
        ...
        df = df.melt(
            id_vars=["fecha", "entidad_id"], value_vars=[1, 2, 3, 4, 5]
        )
        df = df.reset_index()
        df.rename(
            columns={"index": "id", "variable": "tipo", "value": "valor"},
            inplace=True,
        )
        df.to_sql(
