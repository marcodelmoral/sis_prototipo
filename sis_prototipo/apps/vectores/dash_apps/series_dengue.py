...
from sis_prototipo.apps.vectores.dash_apps.utils import (
    ...
    descomposicion_series,
    entidades_opciones_dropdown,
    mapa_init,
)
from sis_prototipo.apps.vectores.models import DatosAgregados
            ...
            [
                dbc.Col(
                    [
                        dcc.Loading(
                            dcc.Graph(
                                id="fig-mapa-burbujas", figure=mapa_init()
                            ),
                            type="cube",
                        ),
                    ]
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        dcc.Loading(
                            dcc.Graph(id="fig-barras", figure={}), type="cube"
                        ),
                    ]
                ),
            ]
...
def llenar_ano_dropdown(_):
    options = [{"label": "Todos", "value": "todos"}] + [{"label": x, "value": x}for x in reversed(list(range(agregados_fecha_dropdown().year,agregados_fecha_dropdown(inicio=False).year+ 1,)))]
    value = "todos"
    return options, value

@app.callback(
    Output("entidad-estacionalidad-dropdown", "options"),
    Output("entidad-estacionalidad-dropdown", "value"),
Input("carga-inicial", "children"),
)
def llenar_entidad_estacionalidad_dropdown(_):
...
@app.callback(
    Output("entidad-analisis-dropdown", "options"),
    Output("entidad-analisis-dropdown", "value"),
Input("carga-inicial", "children"),
)
def llenar_entidad_analisis_dropdown(_):
    options = [
        {
            "label": "Datos "
                     "agregados",
    ...
    ] + entidades_opciones_dropdown(resolver_valor=True, todos=False)
...
def mapa_burbujas_callback(datos_json, ano):
    ...
    if ano == "todos":
        ...
        datos_anuales = (
            datos.groupby(["entidad", "año"])["casos"].sum().reset_index()
        )
...
def serie_agregados_callback(datos_json, tipos):
    ...
    datos_entidades = (
        datos.groupby(["año", "entidad"]).agg(OPT_MAP).reset_index()
    )
...
def serie_analisis_callback(datos_json, entidad, tipo, periodo):
    ...
    if periodo == "año":
        datos = (
            datos.groupby(["año", "entidad"]).agg(OPT_MAP[tipo]).reset_index()
        )
        datos = datos.melt(
    ...
    estacionalidad_string = (
        f"estacional (periodo = {periodo_estacion})"
        if estacionalidad
        else "no estacional"
    )
    estacionariedad_string = (
        "estacionaria" if estacionariedad else "no estacionaria"
    )
    ...
    descomposicion = descomposicion_series(
        datos["valor"], trend=13, low_pass=13
    )
    ...
    return (
        ...
        fig_estacionalidad,
        fig_residuales,
        create_corr_plot(datos["valor"], lags),
