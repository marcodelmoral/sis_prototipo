...
def prepara_datos(
    ...
    """
    vectores = Vector.objects.all().annotate(
        entidad=F("municipio__entidad__nomgeo")
    )
    ...
    if fecha_inicial and fecha_final:
        vectores = vectores.filter(
            fec_sol_aten__range=[fecha_inicial, fecha_final]
        )
    ...
    df["lat"] = df["geom"].apply(lambda x: x.y)
    df["lon"] = df["geom"].apply(lambda x: x.x)
