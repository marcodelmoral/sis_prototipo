...
def entidades_opciones_dropdown(
    resolver_valor: bool = False, todos=True
) -> list[dict[str, str]]:
    """
    TODO: mejorar
    Returns:
    ...
    if resolver_valor:
        try:
            ...
            print(e, "entidades_dropdown")
            entidades = [
                {"label": "No hay entidades", "value": "No hay entidades"}
            ]
    else:
        try:
            entidades = [
    ...
    print(entidades)
    if not entidades:
        entidades = [{"label": "No hay entidades", "value": "No hay entidades"}]
    return entidades
...
def vectores_fecha_dropdown(inicio: bool = True) -> date:
    ...
    # if inicio:
    try:
        return Vector.objects.order_by("fec_sol_aten" if inicio else "-fec_sol_aten")[:1][0].fec_sol_aten
    except Exception as e:
        print(e, "vector fecha")
        return date.today()
    # else:
    #     try:
...
def create_corr_plot(series, lags, plot_pacf=False):
    corr_array = (
        pacf(series.dropna(), alpha=0.05, nlags=lags)
        if plot_pacf
