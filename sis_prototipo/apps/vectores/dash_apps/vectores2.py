...
# Configura
# =======================================================================
from sis_prototipo.apps.geo.serializers import (
    MunicipioGeoCountSerializer,
    MunicipioGeoSerializer,
)
from sis_prototipo.apps.vectores.models import Vector
from sis_prototipo.apps.vectores.serializers import VectorSerializer
...
year_range = list(
    range(qs_fechas[0].fec_sol_aten.year, qs_fechas[1].fec_sol_aten.year + 1)
)
month_range = [
    "Enero",
    "Febrero",
...
app.layout = dbc.Container(
    [
        ...
        dbc.Row(
            dbc.Col(
                html.H1(
                    "Mapa de choro", className="text-center text-primary mb-4"
                ),
                width=12,
            )
        ),
                ...
                dbc.Col(
                    [
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                {"label": x, "value": y}
                                for x, y in zip(month_range, range(1, 13))
                            ],
                        ),
...
def update_choro(entidad, year, month):
    ...
    vectores = vectores.filter(
        fec_sol_aten__year=year, fec_sol_aten__month=month
    )
    # print(vectores)
    if not vectores:
        raise PreventUpdate
    ...
    data = gpd.GeoDataFrame(
        pd.merge(gdf, vectores_count, on="cvegeo", how="left")
    )
    ...
    fig.update_layout(
        mapbox_style="dark", mapbox_accesstoken=settings.MAPBOX_KEY
    )
...
def update_choroo(entidad):
    ...
    muns_count = muns.values(
        "cvegeo", "nomgeo", "vector__fec_sol_aten", "geometry"
    ).annotate(num_casos=Count("vector"))

    geojson = json.dumps(
        MunicipioGeoCountSerializer(muns_count, many=True).data
    )
    gdf = gpd.read_file(geojson)
    print(gdf.columns)
    ...
    gdf = gdf[
        (gdf["vector__fec_sol_aten"].dt.year == 2017)
        & (gdf["vector__fec_sol_aten"].dt.month == 10)
    ]
