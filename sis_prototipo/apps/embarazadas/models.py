...
class Embarazada(models.Model):
    ...
    TURNO = (("Matutino", "Matutino"), ("Vespertino", "Verpertino"))

    creado = models.DateTimeField(
        auto_now_add=True
    )  # fecha de primera atencion?
    ...
    )  # Puede que haya consultorios A, B, C ...
    turno = models.CharField(
        choices=TURNO, blank=True, null=True, max_length=15
    )
    ...
    des_edo_res = models.ForeignKey(
        Entidad, on_delete=models.SET_NULL, null=True, blank=True
    )
    des_mpo_res = models.ForeignKey(
        Municipio, on_delete=models.SET_NULL, null=True, blank=True
    )
    des_loc_res = models.ForeignKey(
        Localidad, on_delete=models.SET_NULL, null=True, blank=True
    )
    geom = models.PointField(
        srid=settings.CRS, null=True, default=None, blank=True
    )
    peso = models.FloatField(
        blank=True, null=True, default=0, help_text="En kilogramos"
    )
    estatura = models.FloatField(
        blank=True, null=True, default=0, help_text="En centímetros"
    )
    # crear una metrica para determinar el numero de gestas vs el numero de paras
    escolaridad = models.CharField(
        choices=ESCOLARIDAD, null=True, blank=True, max_length=20
    )
    ...
    )  # +365 dias para salir del sistema / Fecha probable
    ...
    fecha_envio_re = models.DateField(
        null=True, blank=True
    ...
    antecedente_cu = models.CharField(
        max_length=255, blank=True, null=True
    )  # cirugia uterina
    antecedente_p = models.CharField(
        max_length=255, blank=True, null=True
    )  # perinatales
    periodo_inter = models.CharField(
        max_length=255, blank=True, null=True
    )  # intergenesico
    primer_embarazo_masdecuatro = models.CharField(
        max_length=255, blank=True, null=True
    )  # es pregunta, dropdown?
    antecedente_a = models.BooleanField(
        default=False
    )  # dos o mas abortos espontaneos
    antecedente_c = models.BooleanField(
        default=False
    )  # antecedente de cesaria bool?
    ...
    sangrado_tercer_trimestre = models.BooleanField(
        default=False
    )  # quizas es bool
    polihidramnios = models.BooleanField(
        default=False
    )  # quizas es bool que es?
    ectopico = models.BooleanField(default=False)  # quizas es bool
    enfermedad_tg = models.BooleanField(
        default=False
    ...
    hipertension = models.BooleanField(
        default=False
    )  # a lo mejor añadir valores y brazo
    diabetes = models.BooleanField(default=False)  # quizas es bool
    cardiopatia = models.CharField(
        max_length=255, blank=True, null=True
    )  # congenita - adquirida - na?
    tiroidopatias = models.CharField(max_length=255, blank=True, null=True)
    autoinmune = models.CharField(
        max_length=255, blank=True, null=True
    )  # o de la colagena
    # lupus eritematoso sistemico, artritis reumatoide dropdown ?
    tromboembolica = models.CharField(
        max_length=255, blank=True, null=True
    )  # quizas es bool
    accidente_vascular_cerebral = models.CharField(
        max_length=255, blank=True, null=True
    )  # quizas es bool
    neumopatias = models.CharField(
        max_length=255, blank=True, null=True
    )  # quizas es bool asma bronquial
    tuberculosis = models.CharField(
        max_length=255, blank=True, null=True
    )  # quizas es bool
    hepatopatias = models.CharField(
        max_length=255, blank=True, null=True
    )  # tumores benignos, malignos, hepatitis
    # cirrosis
    ...
    cancer_otros = models.CharField(
        max_length=255, blank=True, null=True
    )  # quizas es bool
    nefropatias = models.CharField(
        max_length=255, blank=True, null=True
    )  # quizas es bool
    ...
    hematologicas = models.CharField(
        max_length=255, blank=True, null=True
    )  # anemia de celula calciformes
    # linfoma, leucemia, purpura trombocitopénica
    enfermedad_isquemica = models.CharField(
        max_length=255, blank=True, null=True
    )  # quizas es bool
    discapacidad = models.CharField(
        max_length=255, blank=True, null=True
    )  # quizas es bool quizas dropdown y descrip
    # mental - fisica - na
    ...
    adicciones = models.CharField(
        max_length=255, blank=True, null=True
    )  # quizas es bool quizas poner cuales o drop

    # PREVENIMSS
    fecha_envio_vi = models.DateField(
        null=True, blank=True
    )  # vacula de influenza
    ...
    vih_reactivo = models.BooleanField(
        default=False
    )  # fue reactiva a una prueba de vih creo que es redundante?
    ivu = models.CharField(
        max_length=255, blank=True, null=True
    )  # que sera esto?
